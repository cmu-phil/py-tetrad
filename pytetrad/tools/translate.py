...
def causal_learn_to_tetrad_graph(cl_graph, col_names):
    """
    Convert a causal-learn GeneralGraph to a Tetrad EdgeListGraph.

    Endpoint encoding (shared by causal-learn and Tetrad/pcalg):
        0 = NULL, 1 = CIRCLE, 2 = ARROW, 3 = TAIL

    For an edge between nodes i and j:
        cl_graph.graph[i][j] = endpoint at j
        cl_graph.graph[j][i] = endpoint at i

    Parameters
    ----------
    cl_graph : causal-learn GeneralGraph
        Output of causal-learn's pc() or similar.
    col_names : list of str
        Variable names in order, e.g. ['X1','X2',...,'X15'].
        Must match the column order used when calling causal-learn.

    Returns
    -------
    Tetrad EdgeListGraph
    """
    p = len(col_names)

    # Build Tetrad node list using the same names as the data columns
    variables = util.ArrayList()
    for name in col_names:
        variables.add(tg.GraphNode(name))

    g = tg.EdgeListGraph(variables)

    ept = {
        1: tg.Endpoint.CIRCLE,
        2: tg.Endpoint.ARROW,
        3: tg.Endpoint.TAIL,
    }

    mat = cl_graph.graph  # numpy int array, shape (p, p)

    # Upper triangle only to avoid adding each edge twice
    for i in range(p):
        for j in range(i + 1, p):
            ep_at_j = ept.get(int(mat[i][j]))  # endpoint at j
            ep_at_i = ept.get(int(mat[j][i]))  # endpoint at i
            if ep_at_j is None or ep_at_i is None:
                continue  # no edge or malformed
            edge = tg.Edge(
                variables.get(i),
                variables.get(j),
                ep_at_i,   # endpoint at node1 (i)
                ep_at_j,   # endpoint at node2 (j)
            )
            g.addEdge(edge)
