# This module wraps the RCIT test from causal-learn in a JPype object so that Tetrad can use it.
...
# that are unhelpful in certain circumstances). So if you want to use the RCIT test from causal-learn,
# you will need to install causal-learn yourself. (See the docs for causal-learn for installation
# instructions.)
#
# To use WrappedClRcit as a test in py-tetrad, you can do the following:
# import pytetrad.tools.WrappedClRcit as wc
# ... load data into a pandas DataFrame df ...
# test = wc.WrappedClRcit(df, alpha=0.01)
#
# This test will implement the IndependenceWrapper interface in Tetrad. For a test that implements the
# IndependenceTest interface in Tetrad, use RcitWrapper instead, in this module.
#
# jdramsey 2024-08-24
...
class RcitWrapper:
    def __init__(self, df, alpha=0.01, start_time=-1, timeout=-1, **kwargs):
        ...
        self.data = df.values
        self.alpha = alpha
        self.rcit_obj = CIT(self.data, "rcit", **kwargs)
    ...
    def checkIndependence(self, *args):
        ...
        pValue = float(self.rcit_obj(X, Y, S))
...
class WrappedClRcit:
    ...
    def getTest(self, *args):
        return RcitWrapper(self.df, alpha=self.alpha, **self.kwargs)

    @JOverride
    def getDescription(self):
        return "Wrapped CL RCIT"
