...
This is still a new project, so please bear with us as we add example modules and clarify the code. Please submit any problems you have figuring things out to our [Issue Tracker](https://github.com/cmu-phil/py-tetrad/issues) and we'll see if we can solve them. Actually, using JPype or causal-learn in Python to integrate Tetrad code is pretty easy once you see how to do it. This is not a complicated project; feel free to steal as needed. :smiley: In fact, the main problem is figuring out what classes are available in Tetrad and what methods to call; we'll try to make that easier. It is a large project. It helps to consult the javadocs for Tetrad. If you'd like to view the javadocs for Tetrad 7.2.2, click [here](https://s01.oss.sonatype.org/content/repositories/releases/io/github/cmu-phil/tetrad-lib/7.2.2/tetrad-lib-7.2.2-javadoc.jar) to download them, then move them into a new directory and unzip them; then view them in a browser. Hopefully, though, we can just add examples for the main things you may want to do.
